...
class HttpAdapter:
    ...
    def handle_client(self, conn, addr, routes):
        ...
        if req.hook:
            ...
            raw_content=hook_result.get("raw_content")
            if raw_content:
                print(f"[HttpAdapter] Building raw content response")
                response = resp.build_json_response(raw_content)
                conn.sendall(response)
                conn.close()
                return
