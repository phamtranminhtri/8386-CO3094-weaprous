...
#                "Cookie": "{}".format(reqhdr.get("Cookie", "sessionid=xyz789")), #dummy cooki
    ...
    def build_json_response(self, data, status_code=200, status_text="OK"):
        """
        Constructs a standard JSON HTTP response.

        :params data (dict): Python dictionary to be sent as JSON.
        ...
        try:
            ...
            return response_header + json_body
        except Exception as e:
            print(f"[Response] Error building JSON response: {e}")
            # Trả về lỗi 500 nếu không thể serialize JSON
            error_msg = b'{"error": "Internal Server Error"}'
            error_header = (
    ...
    def build_response(self, request):
        ...
        if path.endswith('.html') or mime_type == 'text/html':
            ...
            base_dir = self.prepare_content_type(mime_type = mime_type)
        elif mime_type in ['application/json', 'application/xml', 'application/zip', 'application/javascript']:
            base_dir = self.prepare_content_type(mime_type = mime_type)
