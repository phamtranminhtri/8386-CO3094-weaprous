...
@app.route('/broadcast-peer', methods=['GET'])
def broadcast_peer_get(headers, body):
    print(f"[App] broadcast_peer_get with\nHeader: {headers}\nBody: {body}")
    if authenticate(headers):
        return {"auth": "true", "content": "/broadcast-peer.html"}
    return {"auth": "false"}

@app.route('/broadcast-peer', methods=['POST'])
def broadcast_peer_get(headers, body):
    print(f"[App] broadcast_peer_get with\nHeader: {headers}\nBody: {body}")
    if not authenticate(headers): return {"auth": "false"}
    message = body['message']
    msg={"user":get_username(headers),"text":message}
    message_history.append(msg)
    return {"auth": "true", "content": "/broadcast-peer.html"}


@app.route('/update-broadcast-peer', methods=['GET'])
def broadcast_peer_get(headers, body):
    print(f"[App] broadcast_peer_get with\nHeader: {headers}\nBody: {body}")
    if not authenticate(headers):return {"auth": "false"}
    return {"auth": "true", "raw_content": {'username':get_username(headers), 'msg':message_history}}
